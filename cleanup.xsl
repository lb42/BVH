  ...
  <xsl:template match="tei:ref[starts-with(@target, 'n')]">
    <xsl:element name="ref" namespace="http://www.tei-c.org/ns/1.0">
      <xsl:attribute name="target">
        <xsl:value-of
          select="concat('#', concat(substring-before(ancestor::tei:TEI/@xml:id, 'tei'), @target))"
        />
      </xsl:attribute>
      <xsl:apply-templates/>
    </xsl:element>
...
<!-- now deal with @key values -->
  
  <!--  suppress the silly ones -->

  <xsl:template match="tei:msDesc//tei:pubPlace/@key"/>
 <xsl:template match="tei:author/tei:persName/@key" priority="1"/>
  <xsl:template match="tei:msDesc//tei:title/@key"/>

  <!-- we turn name@key to ref unless it is a silly one -->
  ...
  <xsl:template match="tei:text//@key">
    <xsl:attribute name="key">
      <xsl:choose>
        <xsl:when test="starts-with(., '#loc_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#locf_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#loch_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#locm_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#pers_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#persf_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#persh_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="starts-with(., '#persm_')">
          <xsl:value-of select="concat('bvh:', substring-after(., '#'))"/>
        </xsl:when>
        <xsl:when test="matches(., 'B\d+')">
          <xsl:value-of select="concat('bvh:', .)"/>
        </xsl:when>

        <xsl:otherwise>
          <xsl:value-of select="concat('bvh:_', .)"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:attribute>
  </xsl:template>
  ...
  <!-- if only in figure shd be suppressed -->
  ...
  <!-- or make them both into child <ref>s -->
  ...
  <xsl:template match="tei:titlePage/tei:figure/tei:figDesc">
    <xsl:element name="figDesc" namespace="http://www.tei-c.org/ns/1.0">
      <xsl:if test="preceding-sibling::tei:graphic/@xml:base">
        <xsl:element name="ref" namespace="http://www.tei-c.org/ns/1.0">
          <xsl:attribute name="target">
            <xsl:value-of select="preceding-sibling::tei:graphic/@xml:base"/>
          </xsl:attribute>
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:if>
...
<xsl:template match="tei:text/@subtype"/>
  ...
  <!-- only used once and that wrongly -->

  <!--xsl:template match="tei:ab/@rend"/-->

  <xsl:template match="tei:relatedItem"/>
  <!-- all but one are type="original" and that one is empty -->
          ...
          <xsl:attribute name="n">
            <xsl:choose>
              <xsl:when test="following-sibling::tei:fw[@type = 'pageNum'][1]/tei:choice">
                <xsl:value-of
                  select="following-sibling::tei:fw[@type = 'pageNum'][1]/tei:choice/tei:corr/text()"
                />
              </xsl:when>
              <xsl:otherwise>
