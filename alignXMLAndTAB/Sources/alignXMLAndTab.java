...
public class alignXMLAndTab {
	...
	public static void main(String[] args) {
		...
		CheminAlignement calign = new CheminAlignement();
		CheminAlignementPartiel calignBis = null;
    ...
    public static CheminAlignementPartiel editDist3(UTF32String s1, UTF32String s2, int i1, int j1, int i2, int j2) {
	int v1, v2;
	int minOnRow;
	int lgDiag;
	...
	//System.err.println("i1="+i1+" j1="+j1+"     i2="+i2+" j2="+j2);


	//lgDiag = max(20, min(max(I, J) / 20, largeurDiagonale));
	//System.err.println("lgDiag = "+lgDiag);
	
	DiagTable2 dp2 = new DiagTable2(I, J, largeurDiagonale);

	//DiagTable2 dp2 = new DiagTable2(I, J, lgDiag);
	...
	for (int col = 1; col <= I; col++){
		...
		//if (addDiag != 0){
		    if (alternativesLonguesCol != null){
			...
			for (Iterator<UTF32String> i=alternativesLonguesCol.iterator(); i.hasNext();){
			    ...
			    //System.err.println("altern longue  de "+s1.charAt(col-1+i1)+" = "+sAlt+" de longueur :"+sAlt.length());
			    eql = true;
			    ...
			    if (eql){
				//System.err.println("match diagonal 1 en ("+(col-1+i1)+", "+(row-1+i2)+")");
				//System.err.println("alternative : "+sAlt);
				addDiag = 0;
			    }
			    else{
				addDiag = sAlt.length();
		    ...
		    if (alternativesLonguesRow != null){
			for (Iterator<UTF32String> i=alternativesLonguesRow.iterator(); i.hasNext();){
			    ...
			    if (eql){
				addDiag = 0;
				//System.out.println("match diagonal 2");
			    }
			    else{
				addDiag = sAlt.length();
			    }
	...
	CheminAlignementPartiel res=new CheminAlignementPartiel(I+J+1);
	int col=I;
	int row=J;
	int val;
	val = dp2.get(col,row);
	...
	while (col > 0 && row > 0){
	    ...
	    /* Normalement, on pourrait s'épargner les équivalences si vaDiag = dp2.get(col, row) 
	       En effet, on est sûr qu'on ne fera pas mieux :-)
	     */

	    if (!s1.charAt(col-1+i1).equals(s2.charAt(row-1+i2))){
		lequivs =  Translit.longEquiv(s1.charAt(col-1+i1));
		/* On reprend les alternatives de s1.charAt(col-1) */
		if (lequivs != null){
		    for (Iterator<UTF32String> i = lequivs.iterator(); i.hasNext();){
			sAlt = i.next();
			//System.out.println("calcul chemin, alternative de "+s1.charAt(col-1)+" = "+sAlt);
			vaDiag2 = dp2.get(col-1, row-sAlt.length());
			//System.out.println("vaDiag2 = "+vaDiag2+" dp2[col][row] ="+dp2.get(col,row)+" vaDiag = "+vaDiag);
			if (vaDiag2 == dp2.get(col,row)){
			    vaDiag = vaDiag2;
			    //System.out.println("ds calcul chemin, alternative 1");
			    stepMinCol = 1;
		...
		lequivs = Translit.longEquiv(s2.charAt(row-1+i2));
		...
		if (lequivs != null){
			...
			if (vaDiag2==dp2.get(col,row)){
			    //System.out.println("ds calcul chemin, alternative 2");
			    //System.out.println("vaDiag2 = "+vaDiag2+" dp2[col][row] ="+dp2.get(col,row)+" vaDiag = "+vaDiag);
			    // le pb est qu'il faut aussi que l'alternative soit vérifiée i.e. qu'on soit passé par là !
			    vaDiag = vaDiag2;
