...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" version="2.0" xmlns:atilf="http://www.atilf.fr" xmlns="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="atilf tei">
  ...
  <xsl:template match="tei:teiHeader | tei:orig[parent::tei:choice] | tei:sic[parent::tei:choice] | tei:note" mode="passIds">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
  ...
  <xsl:template match="tei:teiHeader | tei:orig[parent::tei:choice] | tei:sic[parent::tei:choice] | tei:note" mode="passNotes">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
      <xsl:attribute name="id" namespace="http://www.atilf.fr">
	<xsl:value-of select="generate-id(.)"/>
